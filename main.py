...
from policies import FirstPolicy, SecondPolicy, ThirdPolicy, MovedType, OnUsageExceeded
...
ThirdPolicy(PROCESSORS_COUNT, generate_tasks(), process_arguments=PROCESS_ARGUMENTS, threshold=THRESHOLD, transfer_treshold=TRANSFER_THRESHOLD, moved=0.5, moved_type=MovedType.USAGE, name="ThirdPolicy(MovedType.USAGE)").run().plot(ax5, PROCESSORS_COLORS)
ThirdPolicy(PROCESSORS_COUNT, generate_tasks(), process_arguments=PROCESS_ARGUMENTS, threshold=THRESHOLD, transfer_treshold=TRANSFER_THRESHOLD, moved=0.5, name="ThirdPolicy(MovedType.COUNT)").run().plot(ax4, PROCESSORS_COLORS)
